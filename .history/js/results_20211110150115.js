...
form.addEventListener('submit', async (e) => {
    ...
    console.log(entries);  
    const elementsHTML = entries.map(([number, counter]) => `<p>$${counter.name}: ${counter.counter} </p></br>`);
    ...
    firestore.collection('search_result').doc('elementsHTML').update({content: "asda"});
    const elementDoc = firestore.collection('search_result').doc('elementsHTML').get('content');
